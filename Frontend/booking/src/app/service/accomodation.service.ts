...
import { accommodation_service_url, modular_url, server } from '../app-global';
...
export class AccommodationService {
  ...
  url = accommodation_service_url + 'api/accommodations';
  ...
  calculatePrice(calculatePrice: CalculatePrice) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/calculate-price`;
    return this._http.post<any>(url,calculatePrice,{headers});
  }

  createReservation(createReservation:CreateReservation){
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations`;
  ...
  createReservationRequest(createReservationRequest:CreateReservationRequest){
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/requests`;
    return this._http.post<any>(url,createReservationRequest,{headers});
  }

  getHostReservationRequest(hostId:string){
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/requests/host/${hostId}`;
    return this._http.get<any>(url,{headers});
  }

  getGuestReservationRequest(guestId: string) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/requests/guest/${guestId}`;
    return this._http.get<any>(url,{headers});
  }

  acceptReservationRequest(id: string) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/requests/accept/${id}`;
    return this._http.put<any>(url,{headers});
  }

  cancelReservationRequest(id: string) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/requests/cancel/${id}`;
    return this._http.put<any>(url,{headers});
  }

  getGuestReservations(id: string) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/guest/${id}`;
    return this._http.get<any>(url,{headers});
  }
  getHostReservations(id: string) {
    const headers = this.loginService.getHeaders();
    const url=`${accommodation_service_url}api/reservations/host/${id}`;
    return this._http.get<any>(url,{headers});
  }
  ...
  executeReservationPayment(request:ExecuteReservationPayment){
    ...
    const url=`${modular_url}api/commerce/reservations/execute-payment`;
  ...
  confirmReservationPayment(request: ExecuteReservationPayment) {
    const headers = this.loginService.getHeaders();
    request.payerId=this.loginService.getCurrentUser().id;
    const url=`${modular_url}api/commerce/reservations/confirm-payment`;
    return this._http.post<any>(url,request,{headers});
  }

  getReservationPaymentStatus(reservationId:string){
    const headers = this.loginService.getHeaders();
    const url=`${modular_url}api/commerce/reservations/payment-status/${reservationId}`;
    return this._http.get<any>(url,{headers});
  }
}
